...
- [spotify-to-mp3](https://github.com/adampaulwalker/spotify-to-mp3-skill) - Turns a Spotify playlist link into tagged 320k MP3s via spotDL, then recovers the tracks that fail the first pass by walking YouTube, SoundCloud, and Bandcamp in order, matching candidates against the Spotify track length. Reports final coverage against the playlist total. Claude Code on macOS only. *By [@adampaulwalker](https://github.com/adampaulwalker)*
