...
- [model-refresh](https://github.com/gupilleveldesigner/model-refresh) - Audits a Claude Code setup (plugins, hooks, agents, skills, MCP connectors, CLAUDE.md) every time a new model ships, judging each item by whether the model can now figure it out unaided. Mutation-tests hooks and completion checks for evidence instead of guessing, verifies runtime effect via headless init events, and gates every change behind explicit approval. *By [@gupilleveldesigner](https://github.com/gupilleveldesigner)*
