...
- [Stickies](https://github.com/dumbspacecookie/stickies) - persistent, expiring, importance-graded sticky notes for Claude Code — zero-turn !!sticky capture, session-start digest, local SQLite (no cloud), opt-in git/Discord sync, plus a cloud/mobile repo-mode. Also on npm (stickies-mcp) and the MCP registry. *By [@dumbspacecookie](https://github.com/dumbspacecookie)*
