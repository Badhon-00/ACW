...
- [Upkeep](https://github.com/wei18/upkeep) - Audits a whole repo for drift — stale docs, specs that no longer match the code, orphaned assets, convention violations — reported with evidence and severity. Output-only (never edits files); runs on a Claude Pro/Max subscription with no API key. Install via `/plugin install upkeep@upkeep` or `npx skills add wei18/upkeep --skill upkeep-audit`, or schedule it as a GitHub Actions workflow. *By [@Wei18](https://github.com/Wei18)*
