...
- [Coding Agent Workspace Seed](./coding-agent-workspace-seed/) - Seeds a safe, reproducible workspace for coding agents contributing to open-source repos with branch setup, scope manifest, and verification commands.
...
- [Micro PR Reviewer](./micro-pr-reviewer/) - Performs fast, focused reviews of small pull requests for open-source contributions before opening or updating a PR.
...
- [OSS Issue Triage](./oss-issue-triage/) - Triages open-source issues to find contribution-ready work with scope estimates, duplicate detection, and claim-or-skip recommendations.
...
- [Repo Safety Gate](./repo-safety-gate/) - Runs pre-flight safety checks before a coding agent modifies a repository including git state, secret scan, and scope boundaries.
