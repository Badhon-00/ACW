...
- [open-bridge](https://github.com/bks-lab/open-bridge) - An agent-agnostic context and orchestration layer that is a plain git repo of Markdown + YAML your coding agent reads at the start of every session, so it starts already knowing your repos, clients, and task conventions — independent of model or frontend. One `skills/` tree works across Claude Code, Codex, and Copilot CLI; ships a persistent world-knowledge context plus a plain-text board and daily log with a backlog→doing→review→done task lifecycle. Single self-hosted instance so far; documents the method, not adoption.
