...
- [SAGE](https://github.com/PsYcGoD/sage) - Local-first CLI wrapper and MCP server that compresses terminal output before it enters agent context. Uses pattern detection to filter test logs, build output, and stack traces while preserving debugging info. Verified 97%+ context reduction across 4,700+ commands with SQLite-backed metrics. Install with `pip install psycgod-sage`. *By [@PsYcGoD](https://github.com/PsYcGoD)*
