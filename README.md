...
- [chmod-777-claude](https://github.com/JeongWonjae/chmod-777-claude) - Three opinionated root-mode skills (rootfix, rootclean, rootbuild) that scope Claude's behavior by intent: root-cause debugging with WHY×5 drilling, behavior-preserving refactoring with one-pattern-per-PR discipline, and pattern-respecting feature additions with mandatory grep + tests. Bilingual (Korean + English) triggers. *By [@JeongWonjae](https://github.com/JeongWonjae)*
