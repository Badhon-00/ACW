...
- [lemmaly](https://github.com/morsechimwai/lemmaly) - Four composable skills (`lemmaly`, `mathguard`, `invariant-guard`, `complexity-cuts`) that force AI coding agents to state Big-O, name the data structure, prove termination, and pick the right algorithm — before writing the loop. Ships a deterministic CI scanner with 59 rules across 11 languages (no LLM, no API key, no network). *By [@morsechimwai](https://github.com/morsechimwai)*
