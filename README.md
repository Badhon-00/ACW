...
- [LaunchMyStore](https://github.com/LaunchMyStore/skill) - Natural-language router for the LaunchMyStore e-commerce platform. Scaffolds apps with the `lms` CLI, dispatches App Bridge actions, builds WASM Functions (cart transform, discount, shipping rate, payment & delivery customization, order validation), writes Aqua/Liquid theme sections, and drives the LaunchMyStore MCP server (130+ merchant-store tools). Universal SKILL.md install — also works in OpenAI Codex, Cursor, Gemini CLI, Windsurf, Antigravity. *By [@LaunchMyStore](https://github.com/LaunchMyStore)*
