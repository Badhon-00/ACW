...
- [LogicLeap Labs](https://github.com/logicleap-labs/claude-plugins) - Six hook-backed guardrail plugins that make AI output actually good: force the model to look at the UI (visual-quality), verify the change ran (verify-before-done), stay in scope (scope-guard), finish the code (no-stub), keep secrets out of source (secret-guard), and cut AI-slop prose (no-slop). Each blocks "done" until the work checks out; `claude plugin marketplace add logicleap-labs/claude-plugins`. *By [@logicleap-labs](https://github.com/logicleap-labs)*
