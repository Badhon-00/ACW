...
- [observent](./observent/) - Production-grade observability for multi-agent Python apps in one command. Auto-detects the agent framework (LangGraph, CrewAI, Microsoft Agent Framework, Anthropic & OpenAI Agents SDK, smolagents, LlamaIndex, Google ADK) and wires up tracing for backends like Arize Phoenix, Langfuse, SigNoz, Elastic APM, LangSmith, Opik, and Jaeger, plus validation and CI eval gates. *By [@HemachandranD](https://github.com/HemachandranD)*
