...
- [fluid](https://github.com/Darcos-Loft/fluid) - An MIT-licensed suite of 6 agent skills that pushes AI frontends away from the generic default look (Inter, purple gradients, default shadcn, pure black) by giving an agent a committed brand identity, a house motion language, and a deterministic 22 rule anti slop detector that runs with no API call. Works with Claude Code, Cursor, Copilot, Codex, Gemini CLI, Windsurf and OpenCode via plain SKILL.md files and pure CSS tokens. *By [@Darcos-Loft](https://github.com/Darcos-Loft)*
