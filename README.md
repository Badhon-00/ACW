...
- [post-change-verification](https://github.com/BWBlueVector/post-change-verification) - Catches the gap between "the file is fixed" and "the running system reflects it" after an AI agent makes a change — a concrete verification discipline instead of trusting the agent's own report. *By [@BWBlueVector](https://github.com/BWBlueVector)*
...
- [suite-workflow](https://github.com/BWBlueVector/suite-workflow) - Turns "give me N of something" into a calibrated Ideate → Build → Self-check → Parallelise batch pipeline, with a pre-committed prediction interview that measures whether the agent actually understands your decision-making before it's trusted to filter anything autonomously. *By [@BWBlueVector](https://github.com/BWBlueVector)*
...
- [obsidian-vault-conventions](https://github.com/BWBlueVector/obsidian-vault-conventions) - Keeps an AI's persistent memory correctly readable by both the AI and a human browsing it in Obsidian — fixes real wikilink and tag bugs that only show up when both "readers" touch the same files. *By [@BWBlueVector](https://github.com/BWBlueVector)*
