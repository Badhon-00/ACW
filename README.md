...
- [SPIDER](https://github.com/baytcho/spider-dead-code-skill) - Finds the code a project does not need and proves it. Splits the whole codebase into individual statements, starts at the entry points and follows the links, so whatever the execution never reaches is reported with its exact file and lines. Python, TypeScript, JavaScript and CSS in one model; a name assembled at runtime is never declared unused; ships with an answer key of 29 checks. *By [@baytcho](https://github.com/baytcho)*
