...
- [Substack Toolkit](./substack-toolkit/) - Full read+write toolkit for Substack from Markdown or Python: draft posts (with image upload), publish Substack Notes (plain or with link attachment), like/comment, walk your reader feed, list any publication's archive, fetch single posts with full body, look up profiles by handle, paginate any user's Notes with engagement metrics, export subscribers. Works around the nine undocumented private-API gotchas (rawHtml editor-crash, base64-only image upload, null bylines on draft PUT, user/self 403, get_post wrapper, notes-for-user filter bug, reader-feed 429s, Note schemaVersion requirement, subscriber export gating). *By [@mphinance](https://github.com/mphinance)*
