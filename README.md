...
- [rote](https://github.com/trevhud/rote) - Compiles a proven skill (a SKILL.md plus references) into a typed, deterministic pipeline. Fixed logic becomes reviewable Python or TypeScript with per-step tests, while judgment steps stay as typed LLM-judge signatures. Emits DBOS, Temporal, Cloudflare Workflows, Inngest, or plain Python/TS, and can serve compiled pipelines back to Claude as MCP tools. Apache-2.0, `pip install rote-cli`. *By [@trevhud](https://github.com/trevhud)*
