...
- [skill-issue](https://github.com/mishanefedov/skill-issue) - Lints coding-agent skills for *activation*: grades each skill's name + description A–F on whether the agent will pick it, simulates which skill a prompt triggers (`--why`), and flags collisions where one skill silently shadows another. Works across Claude Code, Codex, Cursor, opencode, Factory. *By [@mishanefedov](https://github.com/mishanefedov)*
