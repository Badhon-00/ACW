...
- [harness-evolve](https://github.com/AgentGameLab/harness-evolve) - One-pass self-evolution skill for any Agent harness with config + logs: track frontier research → self-audit config → act within L1/L2/L3 risk tiers (safe auto-ship / owner-challengeable / observe-only). Cross-platform — Claude Code, OpenAI Codex, Cursor, Gemini CLI. *By [@AgentGameLab](https://github.com/AgentGameLab)*
...
- [tuneup](https://github.com/AgentGameLab/tuneup) - Translate fuzzy user requests into high-information-density prompts via smart triage (fast / main / ER lanes), 4-quadrant forging (natural / template / XML / Lisp), domain-expert lens presets, and a curated theory-anchor dictionary with a debunked-fallacy blacklist. Triggers implicitly when users keep re-phrasing the same request. Bilingual (中文 + EN). *By [@AgentGameLab](https://github.com/AgentGameLab)*
...
- [learn-coach](https://github.com/AgentGameLab/learn-coach) - Teach-to-mastery pedagogy skill fusing Feynman recall, Socratic questioning, scaffolding, and productive failure. Multi-turn pipeline: diagnose → knowledge map → guess-first → minimum viable model → verify by re-teaching. Every core concept ships with a diagram; jargon always paired with plain-language gloss. Bilingual (中文 + EN). *By [@AgentGameLab](https://github.com/AgentGameLab)*
