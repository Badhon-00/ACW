...
- [opencode-offload](https://github.com/mahmoudayoub/opencode-offload) - Offloads self-contained subtasks (summarization, log triage, batch classification, draft generation) to a local opencode CLI instead of spending Claude tokens, with a free-model-only guard on by default and an optional file-editing agent mode. *By [@mahmoudayoub](https://github.com/mahmoudayoub)*
