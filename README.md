...
- [multivon-eval skills](https://github.com/multivon-ai/multivon-eval/tree/main/multivon_eval/_skills) - Three LLM-evaluation skills that ship inside the multivon-eval Python package (`pip install multivon-eval && multivon-eval install-skills`): /eval-bootstrap generates a calibrated eval suite plus adversarial seed cases from your codebase, /eval-audit runs only the eval cases a PR diff stresses and reports the score delta, /eval-explain explains why an evaluator or threshold was chosen. Every score carries a Wilson 95% CI. *By [@multivon-ai](https://github.com/multivon-ai)*
