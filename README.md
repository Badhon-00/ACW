...
- [dispatch](https://github.com/sparklingneuronics/sparkling-skills/tree/main/plugins/dispatch) - Claude Code plugin for conversationally delegating work to OpenAI Codex CLI and Google Antigravity CLI (multi-model: Gemini, Claude, GPT-OSS). Say "check with codex" or "ask gemini for a second opinion" and Claude runs the other agent, keeps a topic-aware session, and critiques the result. *By [@sparklingneuronics](https://github.com/sparklingneuronics)*
