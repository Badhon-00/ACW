...
- [ghfactory](https://github.com/natejswenson/claude-skills/tree/main/skills/ghfactory) - Generates GitHub Actions workflows that are verified before you ever see the YAML: every action ref resolved to a real pinned SHA via the GitHub API, every `with:` key validated against the action's own action.yml, staleness in majors reported, actionlint and zizmor clean — and it always says exactly which verification rung it reached. *By [@natejswenson](https://github.com/natejswenson)*
