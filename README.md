...
- [dataframe-doctor](https://github.com/Cyriellewu/dataframe-doctor) - Evidence-grounded code review for data-pipeline and dataframe code. Catches silent bugs — dtype coercion, SettingWithCopy, merge fan-out, non-deterministic ordering, timezone bugs, and train/test leakage — and reports each with a `path:line` citation and a minimal fix. Ships a zero-dependency stdlib AST scanner. Works with Claude Code, Cursor, and Codex. *By [@Cyriellewu](https://github.com/Cyriellewu)*
