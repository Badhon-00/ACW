...
- [recap](https://github.com/noluyorAbi/claude-code-recap) - Session manager for Claude Code that lists recent sessions across every project, not just the current directory, with a ready-to-paste `claude -r` resume command, and reopens a whole working set in terminal tabs. Pure Python standard library, offline, read-only. *By [@noluyorAbi](https://github.com/noluyorAbi)*
