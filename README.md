...
- [parallel-agents](https://github.com/unzooai/unterm/tree/master/marketplace/skills/parallel-agents) - Fan a coding task out to 3+ inner AI agents (Claude Code, Codex, Gemini) running in side-by-side Unterm panes — each in its own identity profile so credentials don't collide — then aggregate their diffs. Requires the Unterm MCP server. *By [@unzooai](https://github.com/unzooai)*
...
- [release-engineer](https://github.com/unzooai/unterm/tree/master/marketplace/skills/release-engineer) - End-to-end release engineer: version-bump sweep → tests → tag → push → Apple notarization → CDN manifest republish → live verification. Drives the user's real terminal via Unterm MCP; never auto-merges or force-pushes. *By [@unzooai](https://github.com/unzooai)*
...
- [terminal-control](https://github.com/unzooai/unterm/tree/master/marketplace/skills/terminal-control) - Drive a real macOS / Linux / Windows terminal from the conversation — spawn panes, type into them, read full scrollback, screenshot — via the Unterm MCP server. The base recipe for any agent that needs a real terminal. *By [@unzooai](https://github.com/unzooai)*
