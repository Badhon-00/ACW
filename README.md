...
- [lz-stacks](https://github.com/lutfi-zain/lz-stacks) - A research-backed skill collection for AI coding agents: `lz-session-learn` (5-phase Read–Write reflective loop that distills a session into durable `CLAUDE.md` / `AGENTS.md` / `MEMORY.md` entries, grounded in SAMULE/MARS/Memento 2), `lz-daily-reflect` (end-of-session journaling with decisions, findings, blockers, and next steps), and `lz-create-agentsmd` (3-phase AST/LSP scan → user interview → template inject for full-lifecycle `AGENTS.md` generation). Install all three with `npx skills add lutfi-zain/lz-stacks`. *By [@lutfi-zain](https://github.com/lutfi-zain)*
