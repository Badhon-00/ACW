...
- [Quill](https://github.com/YG3-ai/quill) - Two AIs in conversation — one does the work, the other gives perspective. Four thinking-partner skills: `consult` (stuck/frustrated), `perspective` (exploring), `assumptions` (surface what Claude decided silently), `mosaic` (parallel work + mutual review for high-stakes decisions). Research-tested: 4/4 wins vs single AI. Free on existing Claude Pro + ChatGPT Plus. `pip install quill-mcp`. *By [@YG3-ai](https://github.com/YG3-ai)*
