...
- [meta-prompt](https://github.com/2000silpeed/meta-prompt-skill) - Transforms natural-language requests into prompts optimized for the target model (GPT-5.x, Codex, Claude, Gemini, Nano Banana, Seedance, Higgsfield) using per-model guidebooks distilled from official prompting guides — with freshness tracking, a `refresh` pipeline, and built-in A/B eval. Works in Claude Code and Gemini CLI (docs KR, EN overview). *By [@2000silpeed](https://github.com/2000silpeed)*
