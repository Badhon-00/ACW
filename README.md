...
- [Toolbay Production Audit Skills](https://github.com/sriptcollector/toolbay-skills) - 20 MIT-licensed review skills for the defects that only surface in production: money rounding, webhook replay, unsafe retries, N+1 queries, ungated API routes, timezone boundaries, prompt injection, and destructive migrations. Install the set with `/plugin marketplace add sriptcollector/toolbay-skills`, or one at a time with `npx toolbay add <skill>`. *By [@sriptcollector](https://github.com/sriptcollector)*
