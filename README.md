...
- [Hivemind](https://github.com/activeloopai/hivemind) - Auto-generates skills from your past agent sessions. Captures traces (prompts, tool calls, responses) via hooks, then distills them into SKILL.md files; team workspaces share skills so a lesson learned by one teammate loads for everyone. Works with Claude Code, Codex, Cursor, and OpenClaw. *By [@activeloopai](https://github.com/activeloopai)*
