...
- [schedule-after-usage-reset](https://github.com/lemondepat/schedule-after-usage-reset) - Fetches your Claude usage reset time from the Anthropic API and schedules a deferred task via `/schedule` to run after the limit lifts (with a 5-minute buffer). Install: `/plugin marketplace add lemondepat/schedule-after-usage-reset` then `/plugin install schedule-after-usage-reset@lemondepat`. *By [@lemondepat](https://github.com/lemondepat)*
