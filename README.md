...
- [Bitbucket Jira CLI](https://github.com/Spenhouet/bitbucket-jira-cli) - gh-style CLI for Bitbucket (pull requests, repos, pipelines) and Jira (issues), with branch-name-as-Jira-key automation. Teaches agents to drive the bj CLI (which mirrors gh) and self-installs it if missing; no MCP server needed. *By [@Spenhouet](https://github.com/Spenhouet)*
