...
- [WebCLI](https://github.com/DO-SAY-GO/WebCLI) - Gives AI agents a real, persistent browser with structured page state and numbered actions. The `web` CLI exposes a look→act→re-inspect loop: inspect the page to get numbered refs, act by ref number, re-inspect. Handles multi-tab browsing, file uploads, JS execution, auth handoff, and multi-frame navigation. No screenshots, no brittle selectors. Built on BrowserBox enterprise RBI. Works with Claude Code, Cursor, Codex, and any MCP host. *By [@DO-SAY-GO](https://github.com/DO-SAY-GO)*
