...
- [design-pact](https://github.com/no7z/design-pact) - Makes coding agents follow a project design system: the agent proposes palettes, a local studio deterministically derives a full token set (color/type/spacing/radius/shadow/motion, dark mode) into an agent-executable `design.md` contract, and a `check` command audits generated code for off-contract colors at file:line. Reverse-imports existing codebases via `npx design-pact import`. *By [@no7z](https://github.com/no7z)*
