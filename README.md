...
- [Substack Toolkit](./substack-toolkit/) - Reads and writes Substack from Markdown or Python: draft and publish posts (with image upload), list the published archive, fetch single posts with full body, get publication metadata, export subscribers. Works around the undocumented private-API gotchas (rawHtml editor-crash, base64-only image upload, null bylines on draft PUT, user/self 403, get_post wrapper). *By [@mphinance](https://github.com/mphinance)*
