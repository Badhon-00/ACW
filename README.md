...
- [otelcol-doctor](https://github.com/s3onghyun/otelcol-doctor) - Writes, fixes, and validates OpenTelemetry Collector configs. Encodes the Collector's footguns — processor order (memory_limiter/batch), core-vs-contrib components, pull-vs-push metric exporters, and pipelines that validate but were never wired — and validates with `otelcol validate`. Vendor-neutral. *By [@s3onghyun](https://github.com/s3onghyun)*
