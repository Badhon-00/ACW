...
- [ouroboros](https://github.com/Q00/ouroboros) - Ships 22 skills (`ooo interview`, `ooo seed`, `ooo run`, `ooo evaluate`, `ooo evolve` and more) that run a spec-first loop: a Socratic interview gates the spec on an ambiguity score before code is written, then one command drives execution, a three-stage evaluation gate, and a budgeted evolution loop. The verify command and output assertion used for grading are hidden from the worker, so a struggling agent cannot satisfy the assertion string instead of the criterion. Also an MCP server, and works across 13 runtimes. *By [@Q00](https://github.com/Q00)*
