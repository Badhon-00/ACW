...
- [stipend](https://github.com/stipend-sh/stipend) - Gives an agent its own non-custodial USDC wallet on Base so it can be paid and pay for APIs, with per-transaction, per-day and per-counterparty limits and a destination allowlist enforced in code on the signing path rather than in a prompt.
