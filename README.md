...
- [find-tested-skills](https://github.com/Skillproofdev/find-tested-skills) - Finds, installs, and runs tested Claude skills for your project. Reads the [SkillProof](https://skillproof.dev) catalog of independently installed-and-scored skills (pass/setup/fails verdict, /10 score), shows the best matches, asks which to install, then uses them — never recommending one that scored below plain Claude.
