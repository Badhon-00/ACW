...
- [Proof](https://github.com/EricFinland/proof) - Stop hook + independent verifier that fact-checks an agent's completion claims ("tests pass", "it works", "deployed"). Runs the real checks (tests, build, http, repro) and returns a strict PASS/FAIL/INCONCLUSIVE verdict with command-output receipts. *By [@EricFinland](https://github.com/EricFinland)*
