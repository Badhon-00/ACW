...
- [mozaika-design](https://github.com/sezabut/mozaika-design) - Anti-slop design method for coding agents: decode a real product's measured design system (exact type weights and tracking, color roles with usage rules), lock the spec, build to it, then screen the result against an anti-slop gate. Works standalone; pairs with the Mozaika MCP's free tier. *By [@sezabut](https://github.com/sezabut)*
