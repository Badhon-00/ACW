...
- [echorune-radar](https://github.com/eirik-rune/runemap/tree/main/skills/echorune-radar) - Live weather plus a radar map drawn as text characters for any place on earth, in one HTTP request, distinguishing a genuinely clear cell from one outside radar coverage. Needs network access, so Claude Code rather than the API. *By [@luoshu-echorune](https://github.com/luoshu-echorune)*
