...
Connect the public [TMCRA MCP Server](https://github.com/reshuibuduo/TMCRA-MCP-Server) to the current host. Version 0.5.0 exposes the seven tools used below and is available as a cross-platform MCPB bundle, Python wheel, or GitHub `uvx` package.

The user needs a TMCRA account and API key. Keep the key in the host's secret store or protected environment. Never place it in a prompt, transcript, memory record, or repository.
...
`tmcra_ingest` message objects require `message_id`, `role`, and `content`. `tmcra_turn_prepare` requires stable turn, session, and user-message identifiers. `tmcra_turn_commit` must reuse the prepared turn identifier and add the exact assistant message. The server fixes recall to at most eight evidence windows.
...
**Credit:** Based on the tested lifecycle and tool contracts in [TMCRA MCP Server](https://github.com/reshuibuduo/TMCRA-MCP-Server) and the automatic Codex workflow in [TMCRA Codex Memory](https://github.com/reshuibuduo/TMCRA-Codex-Memory), created by Yu Haoxin / TMCRA.
